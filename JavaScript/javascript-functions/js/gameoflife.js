//complete the function so that it returns its arguments in an array.
function seed() {return [...arguments]};

//test if two cells are the same accept two cells and return boolean value
function same(cell1, cell2) {
  if (JSON.stringify(cell1) === JSON.stringify(cell2)) return true;
  else return false;
}

// The game state to search for `cell` is passed as the `this` value of the function. => WTF does that mean?
